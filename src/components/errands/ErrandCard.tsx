...
import Link from 'next/link'
import { Calendar, Layers, ArrowRight, Users } from 'lucide-react'
import { Progress } from '@/components/ui/progress'
import { Errand } from '@/types/dashboard'
...
export default function ErrandCard({ errand }: ErrandCardProps) {
  ...
  // Calculate Collaborators/Members Count
  const memberCount = errand.members?.length ?? errand._count?.members ?? 0
  const isCollaborative = memberCount > 0
      ...
      <div>
        ...
        <div className='flex items-start justify-between gap-2'>
          <div className='flex flex-col gap-1 max-w-[70%]'>
            <h3 className='font-bold text-slate-900 text-base capitalize tracking-tight truncate'>
              {errand.title.trim()}
            </h3>

            {/* Collaborators Badge */}
            {isCollaborative && (
              <span className='inline-flex items-center gap-1 text-[10px] font-semibold text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-md w-fit'>
        ...
        {/* Optional Subtitle Meta Parameters */}
        {errand.description && (
          <p className='text-xs text-slate-400 mt-2 line-clamp-1 min-h-[16px]'>
            {errand.description}
          </p>
        )}
